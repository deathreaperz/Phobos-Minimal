 ...
 ***/
 ...
 // ASMJIT_PATCH(0x48A634, FlashbangWarheadAt_Details, 5)
 // {
 // 	const auto details = RulesExtData::DetailsCurrentlyEnabled();
 // 	return details ? 0x48A64Au : 0x48A641u;
 // }

ASMJIT_PATCH(0x5FF86E, SpotlightClass_Draw_Details, 5)
{
	const auto details = RulesExtData::DetailsCurrentlyEnabled();
	return details ? 0x5FF87Fu : 0x5FFF77u;
}

ASMJIT_PATCH(0x422FCC, AnimClass_Draw_Details, 5)
{
	const auto details = RulesExtData::DetailsCurrentlyEnabled();
...
ASMJIT_PATCH(0x6D7847, TacticalClass_DrawPixelEffects_Details, 5)
{
	const auto details = RulesExtData::DetailsCurrentlyEnabled();
	return details ? 0x6D7858u : 0x6D7BF2u;
}

ASMJIT_PATCH(0x420F40, AlphaShapeClass_DrawAll_Details, 6)
{
	const auto details = RulesExtData::DetailsCurrentlyEnabled();
	return details ? 0x0u : 0x421346u;
}

ASMJIT_PATCH(0x7609F3, WaveClass_DrawAll_Details, 6)
{
	const auto details = RulesExtData::DetailsCurrentlyEnabled();
